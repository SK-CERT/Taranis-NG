...
| `dictionary`   | Update CPE and CVE dictionaries. | `--upload-cpe`: upload the CPE dictionary (via STDIN in XML format) to the path indicated by `CPE_UPDATE_FILE`<br />`--upload-cve`: upload the CVE dictionary (via STDIN in XML format) to the path indicated by `CVE_UPDATE_FILE`<br />`--upload-cpe-url`: specify the CPE dictionary URL, and update the database from that remote file.<br />`--upload-cve-url`: specify the CVE dictionary URL, and update the database from that remote file.<br />When using `--upload-cpe-url` and `--upload-cve-url`, the remote file can be in plaintext XML or gzipped.<br />The database will be updated automatically after all of the above calls.|
