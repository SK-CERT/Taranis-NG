...
| `dictionary`     | Update CPE and CVE dictionaries. | `--upload-cpe` and `--upload-cve`: upload the CPE and CVE dictionary the path set by the `CPE_UPDATE_FILE` and `CVE_UPDATE_FILE` environment variables. Either upload via xml file piped to stdin or set `CVE_UPDATE_URL` and `CPE_UPDATE_URL` to download and store to `CPE_UPDATE_FILE` and `CVE_UPDATE_FILE`. Database is then updated from these files. |
