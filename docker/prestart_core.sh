...
LD_PRELOAD=/lib/libssl.so.1.1 /app/db_migration.py db upgrade head
echo "Done."

if [[ -n "$TARANIS_NG_SAMPLE_DATA" ]]; then

    LD_PRELOAD=/lib/libssl.so.1.1 /app/manage.py sample-data

elif [[ -n "$TARANIS_NG_ADMIN_USERNAME" && -n "$TARANIS_NG_ADMIN_PASSWORD" ]]; then

    LD_PRELOAD=/lib/libssl.so.1.1 /app/manage.py role \
        --create \
        --name "Admin role" \
    ...
    ROLE_ID=$(LD_PRELOAD=/lib/libssl.so.1.1 /app/manage.py role --list --filter "Admin role" | grep -i id | awk '{print $2}')

    LD_PRELOAD=/lib/libssl.so.1.1 /app/manage.py account --create --username "$TARANIS_NG_ADMIN_USERNAME" --password "$TARANIS_PASSWORD" --roles $ROLE_ID
