...
def get_required_credentials():
    ...
    This function returns the required credentials for authentication.
...
def refresh(user):
    """Refresh the authentication token for the specified user.
...
def check_acl(item_id, acl_check, user):
    ...
    Arguments:
        ...
        acl_check (ACLCheck): The type of ACL check to perform.
        user (User): The user performing the ACL check.
...
def get_id_name_by_acl(acl):
    """Return the corresponding ID name based on the given ACL.

    Arguments:
        acl -- The ACL object.

    Returns:
        The ID name associated with the ACL.
    """
