...
class TraefikSettings(db.Model):
    ...
    Attributes:
        ...
        backend_insecure_skip_verify (bool): Whether Traefik may skip verifying a
            public-web node's own certificate when it proxies to it over HTTPS. Only
            reached in a distributed deployment - a single-host node is plain HTTP on
            the Docker network. Off by default; turning it on trusts any certificate.
        backend_root_cas (str): PEM CA bundle used to verify those node certificates
            instead. Preferred over the switch above: it keeps verification on.
    ...
    backend_insecure_skip_verify = db.Column(db.Boolean(), nullable=False, default=False, server_default="false")
    backend_root_cas = db.Column(db.Text(), nullable=True)
    ...
    def update(cls, data: dict, user_name: str) -> TraefikSettings:
        ...
        record.backend_insecure_skip_verify = bool(new.get("backend_insecure_skip_verify"))
        record.backend_root_cas = (new.get("backend_root_cas") or "").strip()
