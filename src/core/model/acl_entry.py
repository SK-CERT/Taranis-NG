...
class ACLEntry(db.Model):
    ...
    def apply_query(cls, query, user, see, access, modify):
        ...
        if see is False and access is False and modify is False:
            return query.filter(
                or_(ACLEntry.id == None, ACLEntry.everyone == True, ACLEntryUser.user_id == user.id, ACLEntryRole.role_id.in_(roles))
            )
        ...
        if see:
            return query.filter(
                or_(
                    ACLEntry.id == None,
                    and_(
                        ACLEntry.see == True, or_(ACLEntry.everyone == True, ACLEntryUser.user_id == user.id, ACLEntryRole.role_id.in_(roles))
                    ),
                )
            )
        if access:
            return query.filter(
                or_(
                    ACLEntry.id == None,
                    and_(
                        ACLEntry.access == True,
                        or_(ACLEntry.everyone == True, ACLEntryUser.user_id == user.id, ACLEntryRole.role_id.in_(roles)),
                    ),
                )
            )
