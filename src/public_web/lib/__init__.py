...
def handle_exception(
    ...
    if logger:
        # Called from inside an `except` block (it also formats the active traceback
        # below), so .exception() is correct; ruff cannot see across the call.
        logger.exception(message)  # noqa: LOG004
