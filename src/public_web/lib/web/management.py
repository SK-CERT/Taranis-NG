...
# The policy is shared with every other satellite (see shared/auth.py); this node only
# supplies the accessors. taranis_api_key() reads the mounted secret lazily, so it is
# passed as a callable rather than a value.
api_key_required = make_api_key_required(
    config.taranis_api_key,
    lambda: request.headers.get("Authorization"),
)


@management_bp.route("/api/v1/isalive", methods=["GET"], endpoint="isalive_v1")
...
def isalive() -> tuple:
    ...
    Served at ``/api/v1/isalive`` so every satellite answers liveness at one path, and
    still at ``/management/isalive`` so an older core (and this image's own HEALTHCHECK)
    keeps working across an upgrade. The legacy path can go a release later.
    """
