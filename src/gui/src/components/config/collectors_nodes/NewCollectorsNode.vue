<template>
    <v-row v-bind="UI.DIALOG.ROW.WINDOW">
        ...
        <v-dialog v-model="visible" max-width="600" persistent @keydown.esc="cancel">
            <v-card v-bind="UI.DIALOG.BASEMENT">
                ...
                <v-card-text>
                    <v-form @submit.prevent="add" id="form" ref="form">
                        ...
                        <v-text-field :disabled="!canUpdate" :label="$t('collectors_node.name')" name="name" type="text"
                                      v-model="node.name" v-validate="'required'" data-vv-name="name"
                                      :error-messages="errors.collect('name')"
                                      :spellcheck="$store.state.settings.spellcheck"></v-text-field>
                        <v-textarea :disabled="!canUpdate" :label="$t('collectors_node.description')" name="description"
                                    v-model="node.description" :spellcheck="$store.state.settings.spellcheck"></v-textarea>
                        <v-text-field :disabled="!canUpdate" :label="$t('collectors_node.url')" name="url" type="text"
                                      v-model="node.api_url" v-validate="'required'" data-vv-name="url"
                                      :error-messages="errors.collect('url')"></v-text-field>
                        <v-text-field :disabled="!canUpdate" :label="$t('collectors_node.key')" name="key"
                                      :type="showApiKey ? 'text' : 'password'" v-model="node.api_key" v-validate="'required'"
                                      data-vv-name="key" :error-messages="errors.collect('key')"
