                ...
                <v-form @submit.prevent="add" id="form" ref="form" class="px-4">
                    <v-row no-gutters>
                        <v-col cols="6" class="pa-1">
                            <v-text-field :disabled="!canUpdate" :label="$t('user.username')" name="username"
                                type="text" v-model="user.username" v-validate="'required'" data-vv-name="username"
                                :error-messages="errors.collect('username')" />
                        </v-col>
                        <v-col cols="6" class="pa-1">
                            <v-text-field :disabled="!canUpdate" :label="$t('user.name')" name="name"
                                v-model="user.name" />
                        </v-col>
                        <v-col cols="6" class="pa-1">
                            <v-text-field ref="password" :type="showPassword ? 'text' : 'password'" v-model="pwd"
                                v-validate="checkPassEdit ? 'required' : ''"
                                :error-messages="errors.collect('password_check')" :label="$t('user.password')"
                                data-vv-name="password_check" :append-icon="showPassword ? 'mdi-eye-off' : 'mdi-eye'"
                                @click:append="showPassword = !showPassword" />
                        </v-col>
                        <v-col cols="6" class="pa-1">
                            <v-text-field v-model="repwd" :type="showPassword ? 'text' : 'password'"
                                v-validate="checkPassEdit ? 'required|confirmed:password' : ''"
                                :error-messages="errors.collect('password_check')" :label="$t('user.password_check')"
                                data-vv-name="password_check" :append-icon="showPassword ? 'mdi-eye-off' : 'mdi-eye'"
                                @click:append="showPassword = !showPassword" />
                        </v-col>
                    </v-row>
                    ...
                    <v-row no-gutters>
                        <v-col cols="12">
                            <v-data-table :disabled="!canUpdate" v-model="selected_organizations" :headers="headers"
                                :items="organizations" item-key="id" :show-select="canUpdate" class="elevation-1">
                        ...
                        <v-col cols="12" class="pt-2">
                            <v-data-table :disabled="!canUpdate" v-model="selected_roles" :headers="headers"
                                :items="roles" item-key="id" :show-select="canUpdate" class="elevation-1">
                        ...
                        <v-col cols="12" class="pt-2">
                            <v-data-table :disabled="!canUpdate" v-model="selected_permissions" :headers="headers"
                                :items="permissions" item-key="id" :show-select="canUpdate" class="elevation-1">
