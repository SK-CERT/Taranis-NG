            ...
            <v-card v-bind="UI.DIALOG.BASEMENT">
                ...
                <v-form @submit.prevent="add" id="form" ref="form" class="px-4">
                    <v-row no-gutters>
                        ...
                        <v-col cols="12">
                            <v-text-field :disabled="!canUpdate" :label="$t('word_list.name')" name="name" type="text"
                                          v-model="word_list.name" v-validate="'required'" data-vv-name="name"
                                          :error-messages="errors.collect('name')"
                                          :spellcheck="$store.state.settings.spellcheck" />
                        </v-col>
                        <v-col cols="12">
                            <v-textarea :disabled="!canUpdate" :label="$t('word_list.description')" name="description"
                                        v-model="word_list.description" :spellcheck="$store.state.settings.spellcheck" />
                        </v-col>
                    </v-row>

                    <v-row no-gutters>
                        <v-col cols="12">
                            <v-checkbox :disabled="!canUpdate" :label="$t('word_list.use_for_stop_words')"
                                        name="use_for_stop_words" v-model="word_list.use_for_stop_words" />
                        </v-col>
                        <v-col cols="12">
                        ...
                        <v-col cols="12">
                            <v-card style="margin-top: 8px" v-for="(category, index) in word_list.categories"
                                ...
                                <v-card-text>
                                    <v-text-field :disabled="!canUpdate" :label="$t('word_list.name')"
                                                  name="category_name" type="text" v-model="category.name"
                                                  :spellcheck="$store.state.settings.spellcheck"></v-text-field>
                                    <v-textarea :disabled="!canUpdate" :label="$t('word_list.description')"
                                                name="category_description" v-model="category.description"
                                                :spellcheck="$store.state.settings.spellcheck"></v-textarea>

                                    <v-text-field :disabled="!canUpdate" :label="$t('word_list.link')"
                                                  name="category_link" v-model="category.link"></v-text-field>

                                    <WordTable :disabled="!canUpdate" :link="word_list.categories[index].link"
                                               :words="word_list.categories[index].entries" :id="index"
                                               @update-categories="update"></WordTable>
