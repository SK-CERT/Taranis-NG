        ...
        <v-dialog v-bind="UI.DIALOG.FULLSCREEN" v-model="visible">
            <v-card v-bind="UI.DIALOG.BASEMENT">
                ...
                <v-form @submit.prevent="add" id="form" ref="form" class="px-4">
                    <v-row no-gutters>
                        <v-col cols="12">
                            <v-text-field :disabled="!canUpdate" :label="$t('attribute.name')" name="name" type="text"
                                v-model="attribute.name" v-validate="'required'" data-vv-name="name"
                                :error-messages="errors.collect('name')"
                                :spellcheck="$store.state.settings.spellcheck" />
                        </v-col>
                        <v-col cols="12">
                            <v-textarea :disabled="!canUpdate" :label="$t('attribute.description')" name="description"
                                v-model="attribute.description" :spellcheck="$store.state.settings.spellcheck" />
                        </v-col>
                    </v-row>

                    <v-row no-gutters>
                        <v-col cols="6" class="pa-1">
                            <v-combobox :disabled="!canUpdate" v-model="selected_type" :items="types" item-text="title"
                                :label="$t('attribute.type')" />
                        </v-col>
                        <v-col cols="6" class="pa-1">
                            <v-text-field :disabled="!canUpdate" :label="$t('attribute.default_value')"
                                name="default_value" type="text" v-model="attribute.default_value"
                                :spellcheck="$store.state.settings.spellcheck" />
                        </v-col>
                        <v-col cols="6" class="pa-1">
                            <v-combobox :disabled="!canUpdate" v-model="selected_validator" :items="validators"
                                item-text="title" :label="$t('attribute.validator')" />
                        </v-col>
                        <v-col cols="6" class="pa-1">
                            <v-text-field :disabled="!canUpdate" :label="$t('attribute.validator_parameter')"
                                type="text" v-model="attribute.validator_parameter"
                                :spellcheck="$store.state.settings.spellcheck" />
                        </v-col>
                    </v-row>
                    ...
                    <v-row no-gutters>
                        <v-col cols="12">
                            <v-data-table :disabled="!canUpdate" :headers="headers" :items="attribute.attribute_enums"
                                :server-items-length="attribute.attribute_enums_total_count"
                                @update:options="updateOptions" :items-per-page="25" class="elevation-1"
                                :page.sync="current_page" :footer-props="{
                                ...
                                <template v-slot:top>
                                    <v-toolbar flat>
                                        ...
                                        <v-btn v-if="edit && selected_type !== null && selected_type.id === 'CVE'"
                                            color="primary" dark class="mb-2 mr-3" @click="reloadDictionary('cve')">
                                            <v-icon left>mdi-refresh</v-icon>
                                            <span>{{ $t('attribute.reload_cve') }}</span>
                                        </v-btn>
                                        <v-btn v-if="edit && selected_type !== null && selected_type.id === 'CPE'"
                                            color="primary" dark class="mb-2 mr-3" @click="reloadDictionary('cpe')">
                                            <v-icon left>mdi-refresh</v-icon>
                                            <span>{{ $t('attribute.reload_cpe') }}</span>
                                        </v-btn>
                                        <v-btn v-if="edit && selected_type !== null && selected_type.id === 'CWE'"
                                            color="primary" dark class="mb-2 mr-3" @click="reloadDictionary('cwe')">
                                            <v-icon left>mdi-refresh</v-icon>
                                        ...
                                        <v-dialog v-model="importDialog" persistent max-width="290">
                                            <v-card>
                                                <v-card-title class="headline">{{ $t('attribute.reloading')
                                                }}</v-card-title>
                                            ...
                                            <v-card>
                                                ...
                                                <v-row class="ma-6">
                                                    <VueCsvImport v-model="csv" :map-fields="['value', 'description']">
                                                        ...
                                                        <template slot="hasHeaders" slot-scope="{headers, toggle}">
                                                            <label>
                                                                <input type="checkbox" id="hasHeaders" :value="headers"
                                                                    @change="toggle">
                                                ...
                                                <div v-if="csv_preview" class="mt-2 px-4">
                                                    <v-row class="pa-0 ma-0 grey white--text">
                                                        ...
                                                        <v-col class="pa-0 ma-0">
                                                            <span class="heading">{{ $t('attribute.description')
                                                            }}</span>
                                                        </v-col>
                                                    </v-row>
                                ...
                                <template v-slot:item.action="{ item }">
                                    ...
                                    <v-tooltip top>
                                        <template v-slot:activator="{ on, attrs }">
                                            <v-icon v-if="canUpdate" small v-bind="attrs" v-on="on"
...
export default {
    ...
    computed: {
        headers() {
            return [
                { text: this.$t('settings.value'), align: 'left', sortable: false, value: 'value' },
        ...
        formTitle() {
            return this.edited_index === -1 ? this.$t('attribute.add_constant') : this.$t('attribute.edit_constant')
        }
    },
    watch: {
        dialog(val) {
