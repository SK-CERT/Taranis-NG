        ...
        <v-dialog v-bind="UI.DIALOG.FULLSCREEN" v-model="visible">
            <v-card v-bind="UI.DIALOG.BASEMENT">
                ...
                <v-form @submit.prevent="add" id="form" ref="form" class="px-4">
                    <v-row no-gutters>
                        <v-col cols="12">
                            <v-text-field :disabled="!canUpdate" :label="$t('remote_node.name')" name="title"
                                type="text" v-model="remote_node.name" v-validate="'required'" data-vv-name="name"
                                :error-messages="errors.collect('name')"
                                :spellcheck="$store.state.settings.spellcheck" />
                        </v-col>
                        <v-col cols="12">
                            <v-textarea :disabled="!canUpdate" :label="$t('remote_node.description')" name="description"
                                v-model="remote_node.description" :spellcheck="$store.state.settings.spellcheck" />
                        </v-col>
                        <v-col cols="12">
                            <v-text-field :disabled="!canUpdate" :label="$t('remote_node.remote_url')" name="remote_url"
                                type="text" v-model="remote_node.remote_url"
                                :spellcheck="$store.state.settings.spellcheck" />
                        </v-col>
                        <v-col cols="12">
                            <v-text-field :disabled="!canUpdate" :label="$t('remote_node.event_url')" name="events_url"
                                type="text" v-model="remote_node.events_url"
                                :spellcheck="$store.state.settings.spellcheck" />
                        </v-col>
                        <v-col cols="12">
                            <v-text-field :disabled="!canUpdate" :label="$t('remote_node.api_key')" name="api_key"
                                :type="showApiKey ? 'text' : 'password'" v-model="remote_node.api_key"
                    ...
                    <v-row no-gutters>
                        <v-col cols="12">
                            <v-checkbox :disabled="!canUpdate" :label="$t('remote_node.enabled')"
                                v-model="remote_node.enabled" />
                        </v-col>
                        <v-col cols="12">
                            <v-checkbox :disabled="!canUpdate" :label="$t('remote_node.sync_news_items')"
                                v-model="remote_node.sync_news_items" />
                        </v-col>
                        <v-col cols="12">
                            <v-combobox :disabled="!canUpdate" v-model="selected_osint_source_group"
                                :items="osint_source_groups" item-text="name"
                                :label="$t('remote_node.osint_source_group')" />
                        </v-col>
                        <v-col cols="12">
                            <v-checkbox :disabled="!canUpdate" :label="$t('remote_node.sync_report_items')"
