<template>
    <div class="multiselect">
        <v-row class="ml-5 mt-0 mb-2">
            <v-btn :class="multi_select === true ? 'multi-select-button-pressed' : ''" small icon @click.stop="multiSelect" data-btn="multi_select" :title="'Toggle item selection mode'">
            ...
            <v-btn v-if="canModify" small icon :disabled="!multi_select" @click.stop="action('GROUP')" data-btn="group" :title="'Group items'">
                <v-icon small color="white">mdi-group</v-icon>
            </v-btn>
            <v-btn v-if="canModify" class="ml-1" small icon :disabled="!multi_select" @click.stop="action('UNGROUP')" data-btn="ungroup" :title="'Ungroup items'">
                <v-icon small color="white">mdi-ungroup</v-icon>
            </v-btn>
            <v-btn v-if="canCreateReport" class="ml-1" small icon :disabled="!multi_select" @click.stop="analyze" data-btn="analyze" :title="'Analyze items'">
                <v-icon small color="white">mdi-file-outline</v-icon>
            </v-btn>
            <v-btn v-if="canModify" class="ml-1" small icon :disabled="!multi_select" @click.stop="action('READ')" data-btn="read" :title="'Mark items as read'">
                <v-icon small color="white">mdi-eye</v-icon>
            </v-btn>
            <v-btn v-if="canModify" class="ml-1" small icon :disabled="!multi_select" @click.stop="action('IMPORTANT')" data-btn="important" :title="'Mark items as important'">
                <v-icon small color="white">mdi-star</v-icon>
            </v-btn>
            <v-btn v-if="canModify" class="ml-1" small icon :disabled="!multi_select" @click.stop="action('LIKE')" data-btn="like" :title="'Like items'">
                <v-icon small color="white">mdi-thumb-up</v-icon>
            </v-btn>
            <v-btn v-if="canModify" class="ml-1" small icon :disabled="!multi_select" @click.stop="action('DISLIKE')" data-btn="dislike" :title="'Dislike items'">
