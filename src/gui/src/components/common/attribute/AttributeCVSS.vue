<template>
    ...
    <AttributeItemLayout :add_button="addButtonVisible" @add-value="add()" :values="values">
        <template v-slot:content>
            <v-row v-for="(value, index) in values" :key="value.index" class="valueHolder">
                ...
                <AttributeValueLayout v-if="!read_only && canModify && !values[index].remote" :del_button="delButtonVisible"
                    @del-value="del(index)" :occurrence="attribute_group.min_occurrence" :values="values"
                    :val_index="index">
                    <template v-slot:col_left>
                        <CalculatorCVSS :value="calcValue" @update-value="updateValue" @success="report" />
                    </template>
                    <template v-slot:col_middle>
                        <v-text-field v-model="calcValue" :label="$t('attribute.value')" @focus="onFocus(index)"
                            @blur="onBlur(index)" @keyup="directValueChange" :class="getLockedStyle(index)"
                            :disabled="values[index].locked || !canModify" :rules="[rules.vector]"></v-text-field>

                        <v-card class="text-center pb-3" color="white" outlined>
                            <v-row justify="center">
                                <v-col v-for="metric in score.all" :key="metric.name" class="pa-0 mr-1 severity"
                                    :class="metric.severity" style="width: calc(100% / 3); border-radius: 4px;">
                                    <span class="body-2 white--text">{{ $t('cvss_calculator.' + metric.name + '_score') + " "}}
                                    </span>
                                    <span class="body-2 white--text font-weight-bold text-uppercase">{{
...
export default {
    ...
    methods: {
        updateValue(e) {
            ...
            if (value >= 0 && value <= 10) {
                ...
                setTimeout(() => {
                    this.values[0].value = this.score;
                    this.onEdit(0);
                }, 200);
            }
            else {
        ...
        directValueChange() {
            if (this.calcValue >= 0 && this.calcValue <= 10) {
    ...
    mounted() {
        if (this.values[0].value >= 0 && this.values[0].value <= 10) {
