    ...
    export default {
        ...
        methods: {
            ...
            handleStateUpdate(data) {
                // Update card state when state are changed via SSE or NewProduct
                const targetId = data.product_id || data.entity_id;
                if (targetId === this.card.id) {
                    console.log('Updating state for product', targetId, data);
        ...
        mounted() {
            this.$root.$on('product-state-updated', this.handleStateUpdate);
            this.$root.$on('report-item-updated', this.handleStateUpdate);
        },
        beforeDestroy() {
            this.$root.$off('product-state-updated', this.handleStateUpdate);
