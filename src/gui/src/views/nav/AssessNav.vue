    ...
    <v-container class="pa-0">
      ...
      <v-row class="my-4 mr-0 px-5">
        ...
        <v-col cols="12">
          ...
          >
            <template v-slot:selection="{ parent, item, index }">
              <v-chip small v-if="index < 1 && !parent.isMenuActive" @click:close="removeSelectedTopic(item)" label color="grey--lighten-4"
              close close-icon="$newsItemActionRemove" class="pa-2 ml-0 mt-1">
                <span>{{ item }}</span>
              </v-chip>
              <v-chip small v-else-if="parent.isMenuActive" @click:close="removeSelectedTopic(item)" label color="grey--lighten-4"
              close close-icon="$newsItemActionRemove" class="pa-2 ml-0 mt-1">
                <span>{{ item }}</span>
              </v-chip>
              <span
        ...
        <v-col cols="12" class="pt-0">
          ...
          >
            <template v-slot:selection="{ parent, item, index }">
              <v-chip small v-if="index < 1 && !parent.isMenuActive" @click:close="removeSelectedSource(item)" label color="grey--lighten-4"
              close close-icon="$newsItemActionRemove" class="pa-2 ml-0 mt-1">
                <span>{{ item }}</span>
              </v-chip>
              <v-chip small v-else-if="parent.isMenuActive" @click:close="removeSelectedSource(item)" label color="grey--lighten-4"
              close close-icon="$newsItemActionRemove" class="pa-2 ml-0 mt-1">
                <span>{{ item }}</span>
              </v-chip>
              <span
...
export default {
  ...
  data: () => ({
    ...
    topicsList: ['all', 'State', 'Cyberwar', 'Threat', 'DDoS', 'Vulnerability', 'Java', 'CVE', 'OT/CPS', 'Python', 'Privacy', 'Social', 'APT', 'MitM'],
    sourcesList: ['all', 'State', 'Cyberwar', 'Threat', 'DDoS', 'Vulnerability', 'Java', 'CVE', 'OT/CPS', 'Python', 'Privacy', 'Social', 'APT', 'MitM']
  }),
  computed: {
    dateRangeText () {
  ...
  methods: {
    ...
    defaultSource () {
      if (!this.sources.selected.length) {
        ...
        this.sources.selected = this.sources.selected.filter(item => item !== 'all')
      }
    },
    ...
    defaultTopic () {
      if (!this.topics.selected.length) {
        ...
        this.topics.selected = this.topics.selected.filter(item => item !== 'all')
      }
    },
