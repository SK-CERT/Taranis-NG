...
test.describe('Assess report badges', () => {
    ...
    test.beforeAll(async ({ playwright }) => {
        ...
        // IDEMPOTENCY GUARD: the first test creates a news-item aggregate via the GUI with
        // a FIXED title (NEWS_ITEM_TITLE). If a prior run was aborted mid-suite, afterAll
        // never fired and the stale aggregate(s) linger — getByText(NEWS_ITEM_TITLE) then
        // resolves to N elements and Playwright strict mode rejects it. Purge any pre-existing
        // aggregate whose title matches before the suite starts. Also deletes their attached
        // report items so findAggregateIdByTitle returns exactly one row next time.
        await deleteAggregatesByTitle(request, token, (title) => title === NEWS_ITEM_TITLE)
    ...
    test('create a news item via the GUI Add News Item dialog', async ({ page }) => {
        ...
        // The news item should appear in the Assess list. Scope to the first
        // aggregate-card-wrapper so the assertion is unambiguous — getByText alone
        // would match the title in every card and trip Playwright strict mode if any
        // duplicate ever slips past the beforeAll purge.
        const card = page.locator('.aggregate-card-wrapper').filter({ hasText: NEWS_ITEM_TITLE }).first()
