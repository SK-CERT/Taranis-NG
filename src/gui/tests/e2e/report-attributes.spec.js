...
const REPORTS_URL = '/config/reports'
const ANALYZE_URL = '/analyze/local'
...
async function closeAttributeDialog(page) {
    // Cancel, not Escape: a keypress goes to whatever holds focus, and right after pickType()
    // that can still be the Type menu closing outside the dialog, so the dialog's
    // @keydown.esc never fires.
    const dialogs = page.locator('.v-dialog.v-overlay--active')
    await dialogs.first().getByRole('button', { name: 'Cancel' }).first().click()
    // A dirty form raises the unsaved-changes prompt a render later; a clean one just closes.
