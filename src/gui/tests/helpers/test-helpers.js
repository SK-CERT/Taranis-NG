...
/**
 * Absolute backend API base for readiness probes.
 *
 * `waitForBackendReady` polls the backend DIRECTLY — not via the Vite dev-server
 * proxy on :4444. Polling through the proxy couples "is the backend up" to "is
 * Vite's /api proxy initialized", and the proxy can lag Vite's `/`-ready-by-several-
 * seconds during dependency re-optimization on a cold cache (e.g. right after
 * `vite.config.js` changed in a rebase — Vite invalidates its dep cache then).
 * That lag has caused the helper to time out at 60 s while Vite was busy re-optimizing
 * deps, even though the backend itself was healthy. Polling the absolute core URL
 * isolates the backend concern — Vite's own readiness is the webServer's job, not ours.
 */
const E2E_CORE_API = process.env.E2E_CORE_API || `http://127.0.0.1:${process.env.E2E_CORE_PORT || '8090'}/api/v1`

/**
 * Wait for the backend API to be reachable and responsive.
 *
 * The Playwright webServer boots the backend via test-setup.py, but that entry's
 * `url` probe only confirms isalive responded once at boot; cold-cache warming
 * can briefly make subsequent requests slow. This helper polls the backend DIRECTLY
 * at E2E_CORE_API/isalive (not via Vite's /api proxy — see the E2E_CORE_API note
 * above) until it responds 2xx.
 *
 * Implemented with `expect.poll` (Playwright-native polling) so the wait is bounded
 * by the remaining *test* timeout, not by an independent `waitForTimeout` loop that
 * can outlive the test. The test timeout (120 s, see playwright.config.js) is the
 * real upper bound — `timeoutMs` is only a hint for the polling interval.
 * @param {import('@playwright/test').Page} page - Playwright page object (kept for API
 *   parity; the probe uses page.request so it shares the browser context's cookies/CSRF
 *   state even though the URL is absolute and thus bypasses Vite's proxy).
...
export async function waitForBackendReady(page, timeoutMs = 60000) {
    await expect
        .poll(
            async () => {
                try {
                    const res = await page.request.get(`${E2E_CORE_API}/isalive`)
                    return res.ok()
                } catch {
                    return false
                }
            ...
            {
                timeout: timeoutMs,
                intervals: [1000], // poll every 1s
                message: `Backend ${E2E_CORE_API}/isalive did not return 2xx within ${timeoutMs}ms`
            }
        )
        .toBeTruthy()
}
 ...
 * The GUI dev stack keeps two long-lived connections permanently open
