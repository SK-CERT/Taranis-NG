...
/**
 * Delete every news-item aggregate whose `title` matches the predicate.
 *
 * For each matching aggregate, first delete any report items attached to it
 * (via the report_item_news_item_aggregate join table) so the DELETE doesn't
 * leave orphaned ReportItem rows that would resurface across runs and pollute
 * later `findAggregateIdByTitle` lookups. The join rows themselves are removed
 * automatically — migration 57d784d699d9 set ON DELETE CASCADE on the FK.
 *
 * Best-effort: a missing backend, 404s, or unexpected shapes are swallowed.
 * Used by specs that create aggregates via the GUI with FIXED titles
 * (e.g. assess-badges.spec.js's 'E2E Badge Test Item') so the spec is
 * idempotent across aborted runs whose `afterAll` cleanup never fired.
 * @param {import('@playwright/test').APIRequestContext} request
 * @param {string} token
 * @param {(title: string) => boolean} matches
 */
export async function deleteAggregatesByTitle(request, token, matches) {
    const headers = authHeaders(token)
    try {
        // Search broadly — the `search=` param does a substring match on title; we then
        // filter client-side via the predicate so callers can match a prefix or exact title.
        const res = await request.get(`${CORE_API}/assess/news-item-aggregates-by-group/all`, {
            headers,
            params: { offset: 0, limit: 100, sort: 'DATE_DESC', search: '' }
        })
        if (!res.ok()) return
        const body = await res.json()
        const items = body?.items || body?.data?.items || []
        for (const agg of items) {
            const title = String(agg.title ?? '')
            if (!matches(title)) continue
            // Delete attached report items first (best-effort). Resets the join rows on its own.
            try {
                const reportsRes = await request.get(`${CORE_API}/analyze/report-items-by-aggregate/${agg.id}`, { headers })
                if (reportsRes.ok()) {
                    const repBody = await reportsRes.json()
                    const reports = repBody?.items || repBody || []
                    for (const rep of reports) {
                        const repId = rep?.id ?? rep
                        if (repId != null) {
                            await request.delete(`${CORE_API}/analyze/report-items/${repId}`, { headers })
                        }
                    }
                }
            } catch {
                // Swallow — proceed to delete the aggregate regardless.
            }
            await request.delete(`${CORE_API}/assess/news-item-aggregates/${agg.id}`, { headers })
        }
    } catch {
