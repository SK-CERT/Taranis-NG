<template>
    <v-dialog v-model="dialog" max-width="600" persistent>
        ...
        <v-card>
            <v-card-title>
                <span class="text-h5">
                    {{ isEdit ? t('collectors_node.edit') : t('collectors_node.add_new') }}
                </span>
            </v-card-title>
            ...
            <v-card-text>
                <v-form ref="formRef" @submit.prevent="handleSubmit">
                    ...
                    <v-text-field
                        ...
                        :label="t('collectors_node.api_url')"
                        variant="outlined"
                        density="comfortable"
                        class="mb-3"
                        :rules="[(v) => !!v || t('error.required')]"
                    ...
                    <v-text-field
                        ...
                        :label="t('collectors_node.api_key')"
                        variant="outlined"
                        density="comfortable"
                        type="password"
                        :disabled="saving"
...
<script setup lang="ts">
    ...
    const props = withDefaults(
        defineProps<{
            editItem?: Partial<CollectorsNodeItem> | null
        }>(),
        {
            editItem: null
        }
    ...
    const defaultItem: CollectorsNodeItem = {
        id: null,
        name: '',
        description: '',
        api_url: '',
    ...
    const localItem = ref<CollectorsNodeItem>({ ...defaultItem })
    const isEdit = computed(() => !!localItem.value.id)

    const canCreate = computed(() => checkPermission('CONFIG_COLLECTORS_NODE_CREATE'))
    ...
    async function handleSubmit(): Promise<void> {
        ...
        try {
            if (isEdit.value) {
                await updateCollectorsNode(localItem.value)
            } else {
                await createNewCollectorsNode(localItem.value)
            }
            emit('saved')
            handleCancel()
        } catch (error) {
