<template>
    <v-dialog v-model="dialog" max-width="600" persistent>
        ...
        <v-card>
            <v-card-title>
                <span class="text-h5">
                    {{ isEdit ? t('osint_source_group.edit') : t('osint_source_group.add_new') }}
                </span>
            </v-card-title>
            ...
            <v-card-text>
                <v-form ref="formRef" @submit.prevent="handleSubmit">
                    <v-text-field
                        ...
                        :label="t('osint_source_group.name')"
                        variant="outlined"
                        density="comfortable"
                        class="mb-3"
                    ...
                    <v-switch v-model="localItem.default" :label="t('osint_source_group.default')" color="primary" :disabled="saving" />
                </v-form>
...
<script setup lang="ts">
    ...
    async function handleSubmit(): Promise<void> {
        ...
        try {
            if (isEdit.value) {
                await updateOSINTSourceGroup(localItem.value)
            } else {
                await createNewOSINTSourceGroup(localItem.value)
            }
            emit('saved')
            handleCancel()
        } catch (error) {
