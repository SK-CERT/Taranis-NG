                ...
                >
                    ...
                    >
                        <v-window-item value="feed">
                            ...
                            <!-- Host-scoped routing: HSTS and the certificate are
                                 properties of this hostname, so they are set here.
                                 Empty/inherit follows Application Settings ->
                                 Routing & TLS. -->
                            <v-row>
                                <v-col
                                    cols="12"
                                ...
                                >
                                    <v-select
                                        v-model="localItem.hsts"
                                        :items="hstsOptions"
                                        :label="t('public_web.webs.hsts')"
                                        :hint="t('public_web.webs.hsts_hint')"
                                        persistent-hint
                                        variant="outlined"
                                        density="comfortable"
                                        :disabled="saving"
                                ...
                                >
                                    <v-text-field
                                        ...
                                        :label="t('public_web.webs.cert_resolver')"
                                        :hint="t('public_web.webs.cert_resolver_hint')"
                                        persistent-hint
                                        variant="outlined"
                                        density="comfortable"
                                        :rules="[resolverRule]"
                            ...
                            <v-row>
                                <v-col cols="12">
                                    ...
                                    />
                                    <v-textarea
                                        v-model="localItem.tls_cert"
                                        :label="t('public_web.webs.tls_cert')"
                                        :hint="t('public_web.webs.tls_cert_hint')"
                                        persistent-hint
                                        variant="outlined"
                                        density="comfortable"
                                        rows="3"
                                ...
                                <v-col cols="12">
                                    <v-textarea
                                        v-model="localItem.tls_key"
                                        :label="t('public_web.webs.tls_key')"
                                        :hint="t('public_web.webs.tls_key_hint')"
                                        persistent-hint
                                        variant="outlined"
                                        density="comfortable"
                                        rows="3"
                                        :placeholder="
                                            localItem.has_tls_key ? t('public_web.webs.tls_key_stored') : '-----BEGIN PRIVATE KEY-----'
                                        "
                                        :disabled="saving"
                                    />
                                </v-col>
                            ...
                            >
                                ...
                                >
                                    <template
                                        ...
                                        :key="field.key"
                                    >
                                        <!-- Grows with its content instead of being dragged bigger: this is prose
                                             of no fixed length (the service warning is a paragraph or two), and the
                                             dialog is `scrollable`, so the card body takes the extra height while
                                             the toolbar stays put. `rows` is the starting height, not a cap. -->
                                        <v-textarea
...
<script setup lang="ts">
    ...
    // The hostname is what Traefik routes this web on, so it must be a real host
    // name (RFC 1123) — core rejects anything else and would never publish a
    // route for it. Empty is allowed: the web is simply not published yet.
    const HOSTNAME_PATTERN = /^(?=.{1,253}$)([a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?\.)*[a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?$/i

    function hostnameRule(value: string): true | string {
        if (!value) return true
        return HOSTNAME_PATTERN.test(value.trim()) || t('public_web.webs.hostname_invalid')
    }

    // Same shape the backend enforces for the instance-wide resolver name.
    function resolverRule(value: string): true | string {
        if (!value || !value.trim()) return true
        return /^[A-Za-z0-9_-]{1,64}$/.test(value.trim()) || t('routing.cert_resolver_invalid')
    }

    const certSummary = computed(() => {
        if (!localItem.value.tls_cert_subject) return ''
        return t('public_web.webs.tls_cert_summary', {
            subject: localItem.value.tls_cert_subject,
            expires: localItem.value.tls_cert_not_after
        })
    })

    const hstsOptions = computed(() => [
        { title: t('public_web.webs.hsts_inherit'), value: '' },
        { title: t('public_web.webs.hsts_on'), value: 'on' },
        { title: t('public_web.webs.hsts_off'), value: 'off' }
    ])
    ...
    function emptyItem(): WebItem {
        return { id: null, name: '', hostname: '', cert_resolver: '', hsts: '', tls_cert: '', tls_key: '', config: emptyConfig() }
    }
    ...
    function normalize(item: WebItem): WebItem {
        ...
        item.cert_resolver = item.cert_resolver ?? ''
        item.hsts = item.hsts ?? ''
        item.tls_cert = item.tls_cert ?? ''
        // Write-only on the backend: never sent back, so an empty field keeps the
        // stored key and the form starts blank on every open.
        item.tls_key = ''
