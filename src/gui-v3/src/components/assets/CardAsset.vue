<template>
    <v-hover v-slot="{ isHovering, props }">
        <v-card v-bind="props" :elevation="isHovering ? 12 : 2" :class="['asset-card', statusClass]" @click="handleCardClick">
            <v-card-text>
                <v-row align="center" no-gutters>
                    ...
                    <v-col v-if="canDelete" cols="auto" class="delete-btn">
                        <ActionButton action="delete" @click.stop="showDeleteDialog = true" />
                    </v-col>
                </v-row>
            </v-card-text>
