...
describe('Login page', () => {
    ...
    it('does not invent a local password method when the catalog is successfully empty', async () => {
        const { wrapper } = await mountLogin([], false)
        expect(wrapper.vm.hasFormMethods).toBe(false)
        expect(wrapper.find('[data-test="login-username"]').exists()).toBe(false)
    })
