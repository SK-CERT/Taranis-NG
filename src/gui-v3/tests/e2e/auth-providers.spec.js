...
import { login, navigateToConfig, openDialog, saveDialog, generateTestName, findRowByName, activePanel } from '../helpers/test-helpers'
...
test.describe('Login Methods', () => {
    ...
    test('creates a SAML 2.0 provider', async ({ page, request }) => {
        ...
        // Turn the provider on. NewAuthProvider defaults `enabled` to false (configure first,
        // switch on second), and get_saml_authenticator() only builds an authenticator for an
        // ENABLED saml provider — so the /auth/saml/<slug>/metadata assertion further down
        // 404s unless this is checked.
        await dialog.locator('[data-test="auth-provider-enabled"] input').check()
        ...
        // whose response populates the SP certificate + private key fields.
        //
        // On webkit the mouse click on this v-btn (with a prepend-icon) regularly does NOT
        // dispatch the @click handler: the button ends up focused ([active] in the failure
        // snapshot) but no POST is ever made, so click-once + waitForResponse hangs to the
        // test timeout. `force` does not help — Playwright's actionability checks already
        // pass; it is the dispatch that is lost. So drive the button until the fields the
        // response populates are actually filled. Re-generating is side-effect free: the
        // keypair only reaches the database when the dialog is saved.
        const keypairResponses = []
        page.on('response', (response) => {
            if (response.request().method() === 'POST' && response.url().endsWith('/config/auth-providers/saml/generate-keypair')) {
                keypairResponses.push(response)
            }
        })
        const generateButton = dialog.getByRole('button', { name: 'Generate keypair' })
        const spCertificate = dialog.getByLabel('SP certificate', { exact: false }).first()
        const spPrivateKey = dialog.getByLabel('SP private key', { exact: false }).first()

        await expect(async () => {
            await generateButton.click({ force: true })
            await expect(spCertificate).toHaveValue(/-----BEGIN CERTIFICATE-----/, { timeout: 5_000 })
        }).toPass({ timeout: 30_000 })

        // A backend rejection would leave the field empty too, so surface its status rather
        // than letting the retry loop report only "the certificate never appeared".
        for (const response of keypairResponses) {
            expect(response.ok(), `generate-keypair POST failed: ${response.status()}`).toBe(true)
        }
        await expect(spPrivateKey).toHaveValue(/-----BEGIN PRIVATE KEY-----/)
...
test.describe('Auth provider slug contract', () => {
    ...
    test('rejects a second provider whose auto-derived slug would collide', async ({ page, request }) => {
        ...
        try {
            ...
            // the ERROR alert: the OIDC form also renders an informational v-alert with
            // the OAuth callback URI, so a bare `.v-alert` is a strict-mode violation.
            await expect(dialog.locator('.v-alert.bg-error')).toContainText(/could not save|already used/i)
