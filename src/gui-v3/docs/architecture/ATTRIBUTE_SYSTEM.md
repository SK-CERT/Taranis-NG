...
The shared editing logic lives alongside them in `src/components/common/attribute/useAttributes.ts`.
...
```vue
<AttributeContainer :attribute-item="attributeItem" :report-item-id="item.id" :read-only="true" />
```
...
`src/components/common/attribute/useAttributes.ts` provides shared logic used inside each attribute component:
