...
def merge_osint_files(base_folder_path):
    ...
    for root, _, files in os.walk(base_folder_path):
        folder_data = []
        for file in files:
            if file.endswith(".json") and file != f"{category_filename}.json":
                file_path = os.path.join(root, file)
                with open(file_path, "r") as f:
                    json_data = json.load(f)
                    if "data" in json_data:
                        folder_data.extend(json_data["data"])
                        all_merged_data.extend(json_data["data"])
        if folder_data:
            folder_merged_data[root] = folder_data

    # Save merged data for each folder
    for folder, data in folder_merged_data.items():
        merged_file_path = os.path.join(folder, f"{category_filename}.json")
    ...
    if all_merged_data:
        ...
        with open(all_merged_file_path, "w") as f:
            json.dump({"version": version, "data": all_merged_data}, f, indent=4)
